...
std::unique_ptr<Node::AST> Parser::dispatch() {
  ...
  if (peek().type == GenericToken::IDENTIFIER) {
    if (symbol_table_.findFunction(peek().value)) {
      return createASTNode([this] { return functionCall(); },
                           "null function call");
    } else {
      std::cerr << "received: '" << peek().value << '\'' << std::endl;
      throw std::invalid_argument(
          "syntax error: unrecognized literal in expression");
    }
  }
  ...
  if (value == "fn") {
    return createASTNode([this] { return functionDeclaration(); },
                         "null function declaration");
  }
  if (value == "fn_") {
    return createASTNode([this] { return voidFunctionDeclaration(); },
                         "null function declaration");
  }
  if (value == "let" || value == "op") {
    return createASTNode([this] { return variableDeclaration(); },
                         "null variable declaration");
  }
  if (value == "if") {
    return createASTNode([this] { return conditionalStatement(); },
                         "null conditional statement");
  }
  if (value == "rev" || value == "ret") {
    return createASTNode([this] { return returnStatment(); },
...
std::shared_ptr<Node::VariableDecl> Parser::variableDeclaration() {
  ...
  require(peek().value == "let",
          "error: expected keyword in variable declaration");

  pop();

  require(peek().type == GenericToken::IDENTIFIER,
  ...
  require(peek().type == GenericToken::TYPE,
          "error: expected type in variable declaration");
  require(dictionary_.find(peek().value) != dictionary_.end(),
          "error: unrecognized type in variable declaration");
  ...
  require(tokens_.empty() || peek().value == ";",
          "expected newline in expression");
  return declaration;
}
...
std::shared_ptr<Node::FunctionDecl> Parser::functionDeclaration() {
  ...
  require(peek().value == "fn",
          "error: expected keyword in function declaration");
  ...
  require(peek().value == "(",
          "error: expected punctuator in function declaration");
  std::vector<std::pair<std::string, ConstituentToken>> parameters =
      parameterlist();
  ...
  require(dictionary_.find(return_type) != dictionary_.end(),
          "error: unrecognized type");
  function_decl.return_type = dictionary_.at(return_type);
  function_decl.body = dispatch();
  symbol_table_.exitScope();
  auto function =
      std::make_shared<Node::FunctionDecl>(std::move(function_decl));
...
std::unique_ptr<Node::ConditionalStatement> Parser::conditionalStatement() {
  ...
  if (token.value == "else") {
    ...
    throw std::invalid_argument(
        "error: expected keyword in conditional expression");
  }
...
std::unique_ptr<Node::GenericExpr> Parser::recurseFactor() {
  ...
  if (peek().value == "!" || peek().value == "~") {
    ...
    un_expr.op = (op == "!") ? ConstituentToken::LOGICAL_NOT
    ...
    return std::make_unique<Node::GenericExpr>(Node::GenericExpr{
...
std::unique_ptr<Node::GenericExpr> Parser::recurseAdditive() {
  ...
  while (!tokens_.empty() && (peek().value == "+" || peek().value == "-")) {
    ...
    bin_expr.op = (peek().value == "+") ? ConstituentToken::ARITHMETIC_ADD
...
void Parser::debugExprPrinterRecursive(Node::GenericExpr &node, int depth) {
  ...
  if (std::holds_alternative<Node::GenericExpr>(*variant)) {
    ...
    std::cout << indent << "holds binary expression: "
              << reverse_dictionary_.at(binaryExpr.op) << std::endl;
    if (!binaryExpr.lhs || !binaryExpr.rhs) {
      throw std::invalid_argument(
          "error: missing operand in binary expression");
    }
    ...
    std::cout << indent << "holds unary expression: "
              << reverse_dictionary_.at(unaryExpr.op) << std::endl;
    if (unaryExpr.expr) {
      debugExprPrinterRecursive(*unaryExpr.expr, depth + 1);
    }
  } else if (std::holds_alternative<Node::NumericLiteral>(*variant)) {
    std::cout << indent << "holds numeric literal: "
              << std::get<Node::NumericLiteral>(*variant).token.value
              << std::endl;
  } else if (std::holds_alternative<Node::Identifier>(*variant)) {
    std::cout << indent << "holds identifier: "
              << std::get<Node::Identifier>(*variant).token.value << std::endl;
  } else if (std::holds_alternative<Node::StringLiteral>(*variant)) {
    std::cout << indent << "holds string literal: \""
              << std::get<Node::StringLiteral>(*variant).token.value << "\""
              << std::endl;
  } else if (std::holds_alternative<Node::FunctionCall>(*variant)) {
    const auto &functionCall = std::get<Node::FunctionCall>(*variant);
    std::cout << indent << "holds function call: " << functionCall.token.value
              << std::endl;
