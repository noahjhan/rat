...
std::shared_ptr<Node::GenericExpr> Parser::functionCall()
{
    ...
    if (function_dictionary_.find(call.token.value) == function_dictionary_.end()) {
        ...
        throw std::invalid_argument(
        "function dictionary does not recognize function call");
    }
...
std::shared_ptr<Node::ReturnStatement> Parser::returnStatment()
{
    if (peek().value == "rev") {
        ...
        if (peek().value != ";") {
            std::cerr << "received '" << peek().value << '\'' << std::endl;
            throw std::invalid_argument(
            "error: expecting newline after return statement");
        }
...
std::shared_ptr<Node::GenericExpr> Parser::recurseFactor()
{
    ...
    if (peek().value == "!" || peek().value == "~") {
        ...
        un_expr.op =
        (op == "!") ? ConstituentToken::LOGICAL_NOT : ConstituentToken::BITWISE_NEG;
...
void Parser::debugExprPrinterRecursive(Node::GenericExpr &node, int depth)
{
    ...
    if (std::holds_alternative<Node::GenericExpr>(*variant)) {
        ...
        std::cout << indent
                  << "holds binary expression: " << reverse_dictionary_.at(binaryExpr.op)
                  << std::endl;
        ...
        std::cout << indent
                  << "holds unary expression: " << reverse_dictionary_.at(unaryExpr.op)
                  << std::endl;
        if (unaryExpr.expr) {
        ...
        std::cout << indent << "holds numeric literal: "
                  << std::get<Node::NumericLiteral>(*variant).token.value << std::endl;
    }
